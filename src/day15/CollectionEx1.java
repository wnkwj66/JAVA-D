...
//		<Num> Num 클래스의 객체만 저장 가능한 list를 ArrayList로 생성
		ArrayList<Num> list = new ArrayList<Num>();
		list.add(new Num());
//		Ch클래스를 이용한 객체는 Num클래스를 이용한 객체가 아니므로 에러 발생.
//		list.add(new Ch()); // 에러
//		Num2는 Num클래스를 상속받았기 때문에 에러 발생 x (자식 클래스를 부모 클래스로 자동 형변환 할 수 있기 때문)
//		list.add(new Num2()); //정상
		list.add(new Num());
		list.add(new Num(1));
		...
		printList(list);
		list2.clear();// 지우는거 : list2 의 주소를 삭제
		System.out.println("list2를 clear 한 후 list2");
		printList(list2); 
		list2.addAll(list); //안에 내용 다 추가
		System.out.println("list를 list2에 추가 후 lis2");
		printList(list2);
		...
		while(it.hasNext()) { //hasNext는 다음 값이 있는지 없는지만 확인
			Num tmp = it.next(); //it.next가 다음 값을 가져옴
			System.out.print(tmp +" ");
		}
		System.out.println();
		list.ensureCapacity(100); //미리 만들어놓는 배열이 최소 100개가 되도록 하는 메서드 .size랑 다른 개념
//		size는 list에 저장된 객체의 개수? 크기?
		List<Num>list3 = list.subList(0, 2);
		System.out.println("0번지에서 2번지 사이의 부분리스트(2번지 제외)");
		printList(list3);
//		아래는 익명클래스,익명클래스는 주로 메서드에서 인터페이스를 구현한 클래스가 필요한데
//		해달 메소드가 기능이 하나 뿐이어서 따로 정의하기 귀찮은 경우 익명 클래스 이용
